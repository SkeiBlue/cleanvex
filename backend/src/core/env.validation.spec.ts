...
describe('checkEnv', () => {
  it('ok quand les vars requises sont présentes', () => {
    const r = checkEnv({ DATABASE_URL: 'x', JWT_ACCESS_SECRET: 'y' } as NodeJS.ProcessEnv);
    expect(r.ok).toBe(true);
    expect(r.missing).toEqual([]);
  });

  it('liste les vars manquantes', () => {
    const r = checkEnv({} as NodeJS.ProcessEnv);
  ...
  it('considère une string vide comme absente', () => {
    const r = checkEnv({ DATABASE_URL: '   ', JWT_ACCESS_SECRET: 'y' } as NodeJS.ProcessEnv);
    expect(r.missing).toContain('DATABASE_URL');
  });
});
...
describe('assertRequiredEnv', () => {
  it('ne throw pas si vars présentes', () => {
    expect(() =>
      assertRequiredEnv({ DATABASE_URL: 'x', JWT_ACCESS_SECRET: 'y' } as NodeJS.ProcessEnv),
    ).not.toThrow();
  });

  it('throw avec la liste si vars manquantes', () => {
    expect(() => assertRequiredEnv({} as NodeJS.ProcessEnv)).toThrow(/DATABASE_URL.*JWT_ACCESS_SECRET/);
  });
});
