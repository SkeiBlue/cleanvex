...
describe('ErrorLoggingFilter', () => {
  it('renvoie 400 pour une HttpException', () => {
    ...
    filter.catch(new HttpException('Bad input', HttpStatus.BAD_REQUEST), makeHost(statusFn, jsonFn) as never);
    expect(statusFn).toHaveBeenCalledWith(400);
    expect(jsonFn).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 400 }));
  });
  ...
  it('inclut timestamp et path dans la reponse', () => {
    ...
    const statusFn = jest.fn().mockReturnValue({ json: jsonFn });
    const filter = new ErrorLoggingFilter(makePrisma() as never);
    filter.catch(new HttpException('not found', 404), makeHost(statusFn, jsonFn) as never);
  ...
  it('loggue en DB les erreurs 500', () => {
    ...
    filter.catch(new Error('crash serveur'), makeHost(statusFn, jsonFn) as never);
